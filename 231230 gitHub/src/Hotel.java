	...
	public int[] hotel(boolean isCleaner) {
		...
		if (isCleaner) {
			try {
				...
				} else if ("".equals(RF[cFloor - 2][cUnit - 1].getRoomStatus()) || RF[cFloor - 2][cUnit - 1].getRoomStatus().isEmpty()) {
		...
		while (go) {
			...
			case 2:
				...
				for (int i = 0; i < reservationList.length; i++) {
					...
					} else if (reservationList[i] != null && reservationList[i].getPhoneNumber().equals(reservationPhoneNumber)) {
					} else {/*
